...
// 특정 여행 정보 가져오기(예산 정보 포함)
export async function getTripByTripId(req, res) {
  const { tripId } = req.params;
  const userId = req.user.id;
  try {
    const trip = await tripRepository.findTripsByUserId(userId);
...
export async function joinTripByInvite(req, res) {
  ...
  try {
    ...
    const tripId = trip._id.toString(); // query로 넘기기 위해 문자열로 변환
...
export async function leaveTrip(req, res) {
  try {
    ...
    const result = await tripRepository.leaveTripAsCollaborator(tripId, userId);
    ...
    if (result.modifiedCount === 0) {
      return res
        .status(400)
        .json({ message: "협업자가 아니거나 이미 나갔습니다." });
    }

    return res.json({ message: "나가기 완료" });
  } catch (err) {
    if (err.message === "OWNER_CANNOT_LEAVE") {
      return res.status(400).json({ message: "owner는 나갈 수 없습니다." });
    }
    console.error("leaveTrip error:", err);
    return res.status(500).json({ message: "나가기 실패" });
  }
}
